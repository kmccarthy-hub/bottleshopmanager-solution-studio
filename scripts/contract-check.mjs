...
assert.match(stageRepairGuidance("maker", new Error('Artifact validation failed: Maker interactions cannot use dynamic code execution. Blocked token: "Function(".')), /ordinary lowercase function declaration/);
assert.match(stageRepairUserMessage("maker", new Error('Artifact validation failed: Maker interactions cannot use dynamic code execution. Blocked token: "Function(".')), /dynamic code-execution constructor/);
...
assert.throws(() => validateDownstreamArtifact("maker", { ...maker, prototypes: maker.prototypes.map((item) => ({ ...item, prototypeScript: `${item.prototypeScript} const runGenerated=new Function("return true");` })) }, "run-1", artifacts), /dynamic code execution.*Blocked token: "new Function\("/);
